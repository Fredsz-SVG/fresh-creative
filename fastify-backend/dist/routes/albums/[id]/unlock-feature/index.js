...
const route = async (server) => {
    ...
    server.post('/', async (request, reply) => {
        ...
        const { data: existing } = await admin
            ...
            .maybeSingle();
        if (existing)
            return reply.code(409).send({ error: 'Fitur sudah dibuka sebelumnya.' });
        ...
        if (cost <= 0)
            return reply.code(400).send({ error: 'Fitur tidak dapat dibuka dengan credit.' });
        ...
        const { error: updateErr } = await admin
            ...
            .eq('id', user.id);
        if (updateErr)
            return reply.code(500).send({ error: 'Gagal memotong credit.' });
