...
async function handleRedeem(userId, code, reply) {
    if (!code || !code.trim()) {
    ...
    if (!redeemCode)
        return reply.code(404).send({ error: 'Kode tidak ditemukan.' });
    if (!redeemCode.is_active)
        return reply.code(410).send({ error: 'Kode sudah tidak aktif.' });
    ...
    const { data: existing } = await admin
        ...
        .maybeSingle();
    if (existing)
        return reply.code(409).send({ error: 'Kamu sudah pernah menggunakan kode ini.' });
    ...
    if (updateErr)
        return reply.code(500).send({ error: 'Gagal menambah credit.' });
...
async function handleCreate(body, reply) {
    ...
    if (error) {
        if (error.code === '23505')
            return reply.code(409).send({ error: 'Kode sudah ada, gunakan kode lain.' });
...
const route = async (server) => {
    ...
    server.put('/', async (request, reply) => {
        ...
        if (typeof credits === 'number' && credits > 0)
            updateObj.credits = credits;
        if (typeof max_uses === 'number' && max_uses >= 1)
            updateObj.max_uses = max_uses;
        if (typeof is_active === 'boolean')
            updateObj.is_active = is_active;
        if (expires_at !== undefined)
            updateObj.expires_at = expires_at || null;
