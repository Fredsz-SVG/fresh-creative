...
const route = async (server) => {
    ...
    async function verifyAdmin(request, reply) {
        ...
        return { user, adminClient: (0, supabase_1.getAdminSupabaseClient)() };
    }
    ...
    server.get('/', async (request, reply) => {
        ...
        const pageParam = parseInt(request.query?.page ?? '1', 10);
        const perPageParam = parseInt(request.query?.perPage ?? '10', 10);
        ...
        try {
            ...
            if (!authUsers.error && authUsers.data?.users?.length) {
                const rows = authUsers.data.users.map((u) => ({
            ...
            const totalCredits = (creditRows ?? []).reduce((sum, row) => sum + (typeof row.credits === 'number' ? row.credits : 0), 0);
    ...
    server.put('/', async (request, reply) => {
        ...
        if (typeof credits === 'number') {
            if (credits < 0)
                return reply.code(400).send({ error: 'Credits must be >= 0' });
