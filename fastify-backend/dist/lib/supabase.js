    ...
    if (!rawCookie && process.env.NEXT_PUBLIC_SUPABASE_URL) {
        try {
            ...
            if (!rawCookie) {
                ...
                let chunkStr = '';
                for (let i = 0; i < 5; i++) {
                    if (cookies[`${authKey}.${i}`])
                        chunkStr += cookies[`${authKey}.${i}`];
                }
                if (chunkStr)
                    rawCookie = chunkStr;
            }
    ...
    if (rawCookie) {
        try {
            ...
            if (parsed?.access_token)
                ...
                token = typeof parsed[0] === 'string' ? parsed[0] : parsed[0].access_token || parsed.access_token;
        }
        catch {
            if (typeof rawCookie === 'string' && rawCookie.startsWith('eyJ'))
                token = rawCookie;
        }
    }
