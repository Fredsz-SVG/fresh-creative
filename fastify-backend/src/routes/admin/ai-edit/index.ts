...
const route: FastifyPluginAsync = async (server) => {
    ...
    server.put('/', async (request: any, reply) => {
        ...
        if (error) return reply.code(500).send({ error: error.message })
        if (!data || data.length === 0) return reply.code(404).send({ error: 'Pricing not found' })
    ...
    server.post('/', async (request: any, reply) => {
        ...
        const body = request.body || {}
        const featureSlug = (body.feature_slug as string)?.trim()
        const units = typeof body.units === 'number' && body.units > 0 ? Math.floor(body.units) : 1
        if (!featureSlug) return reply.code(400).send({ ok: false, error: 'feature_slug is required' })
        ...
        if (pe) return reply.code(500).send({ ok: false, error: pe.message })
        ...
        if (ue) return reply.code(500).send({ ok: false, error: ue.message })
        ...
        if (cost > 0) {
            ...
            if (upErr) return reply.code(500).send({ ok: false, error: upErr.message })
        }
