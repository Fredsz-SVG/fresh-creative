...
const route: FastifyPluginAsync = async (server) => {
  server.get('/', async (request: any, reply: any) => {
    ...
    const classId = (request.query as any)?.class_id
    const studentName = (request.query as any)?.student_name
  
    if (!classId) return reply.code(400).send({ error: 'class_id required' })
    ...
    try {
      ...
      for await (const part of parts) {
        if (part.type === 'file' && part.fieldname === 'file') {
          ...
          if (part.fieldname === 'class_id') classId = String(part.value ?? '').trim()
          if (part.fieldname === 'student_name') studentName = String(part.value ?? '').trim()
        }
      }
    } catch (err) {
