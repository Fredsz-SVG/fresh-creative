...
const route: FastifyPluginAsync = async (server) => {
  server.post('/', async (request: any, reply) => {
    try {
      ...
      if (!subjects || !Array.isArray(subjects) || subjects.length < 2) return reply.code(400).send({ ok: false, error: 'Minimal 2 gambar' })
      if (subjects.length > 10) return reply.code(400).send({ ok: false, error: 'Maksimal 10 gambar' })
      if (!(body.prompt || '').trim()) return reply.code(400).send({ ok: false, error: 'Prompt wajib diisi!' })
      ...
      if (!result) return reply.code(500).send({ ok: false, error: 'Tidak ada hasil' })
