...
const route: FastifyPluginAsync = async (server) => {
  server.post('/', async (request: any, reply) => {
    try {
      ...
      if (!body.image) return reply.code(400).send({ ok: false, error: 'File foto tidak valid' })
      ...
      if (!videoUrl) return reply.code(500).send({ ok: false, error: 'Tidak ada hasil video' })
