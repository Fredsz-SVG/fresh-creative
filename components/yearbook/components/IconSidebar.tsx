...
function IconSidebarInner({
  ...
  return (
    <div className="hidden lg:fixed lg:left-0 lg:top-14 lg:w-16 lg:h-[calc(100vh-3.5rem)] lg:flex flex-col lg:z-40 lg:bg-white lg:dark:bg-slate-900 lg:border-r-2 lg:border-slate-200 lg:dark:border-slate-700 lg:shadow-[2px_0_0_0_#0f172a] lg:dark:shadow-[2px_0_0_0_#334155]">
      {canManage && (
      ...
      <a href={url('ai-labs')} className={`relative ${linkClass(sidebarMode === 'ai-labs')}`} title="AI Labs" onClick={handleClick('ai-labs')}>
        <div className="relative">
          ...
          {!aiLabsAccessible && (
            <Lock className="w-3.5 h-3.5 absolute -top-1.5 -right-1.5 text-purple-600 dark:text-purple-400 bg-white dark:bg-slate-800 rounded-full p-0.5 border border-slate-200 dark:border-slate-600 shadow-[4px_4px_0_0_#334155] dark:shadow-[4px_4px_0_0_#1e293b]" />
          )}
        </div>
        <span>AI Labs</span>
      </a>
      ...
      <a href={url('flipbook')} className={`relative ${linkClass(sidebarMode === 'flipbook')}`} title="Flipbook" onClick={handleClick('flipbook')}>
        <div className="relative">
          ...
          {!flipbookAccessible && (
            <Lock className="w-3.5 h-3.5 absolute -top-1.5 -right-1.5 text-amber-600 dark:text-amber-400 bg-white dark:bg-slate-800 rounded-full p-0.5 border border-slate-200 dark:border-slate-600 shadow-[4px_4px_0_0_#334155] dark:shadow-[4px_4px_0_0_#1e293b]" />
          )}
        </div>
        <span>Flipbook</span>
      </a>
      {canManage && (
        <a href={url('approval')} className={`relative ${linkClass(sidebarMode === 'approval')}`} title="Kelola Approval" onClick={handleClick('approval')}>
          ...
          <span>Approval</span>
          {pendingCount > 0 && (
            <span className="absolute top-3 right-3 flex h-3 w-3 rounded-full bg-red-500 border-2 border-slate-200 dark:border-slate-700 shadow-[4px_4px_0_0_#334155] dark:shadow-[4px_4px_0_0_#1e293b] animate-pulse" />
          )}
        </a>
      )}
    </div>
