...
const linkClass = (active: boolean) =>
  `flex-shrink-0 flex flex-col items-center justify-center gap-1.5 py-4 border-b-2 border-black dark:border-slate-700 text-[10px] font-black uppercase tracking-tight transition-all w-full ${active
...
function IconSidebarInner({
  ...
  return (
    <div className="hidden lg:fixed lg:left-0 lg:top-14 lg:w-16 lg:h-[calc(100vh-3.5rem)] lg:flex flex-col lg:z-40 lg:bg-white lg:dark:bg-slate-900 lg:border-r-2 lg:border-black lg:dark:border-slate-700 animate-in fade-in slide-in-from-left-4 duration-500">
      {canManage && (
      ...
      <a href={url('ai-labs')} className={`relative ${linkClass(sidebarMode === 'ai-labs')}`} title="AI Labs" onClick={handleClick('ai-labs')}>
        <div className="relative">
          ...
          {!aiLabsAccessible && (
            <Lock className="w-3.5 h-3.5 absolute -top-1.5 -right-1.5 text-slate-900 dark:text-white bg-white dark:bg-slate-800 rounded-full p-0.5 border-2 border-black dark:border-slate-700" />
          )}
        </div>
        <span>AI Labs</span>
      </a>
      ...
      {canManage && (
        <a href={url('approval')} className={`relative ${linkClass(sidebarMode === 'approval')}`} title="Kelola Approval" onClick={handleClick('approval')}>
          ...
          <span>Approval</span>
          {pendingCount > 0 && (
            <span className="absolute top-3 right-3 flex h-3 w-3 rounded-full bg-red-500 border-2 border-black dark:border-slate-700 animate-pulse" />
          )}
        </a>
      )}
    </div>
