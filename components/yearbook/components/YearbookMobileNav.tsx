...
export default function YearbookMobileNav({
  ...
  return (
    <>
      ...
      {!hideBottomNav && (
        <div className={`fixed bottom-0 left-0 right-0 z-[60] bg-white dark:bg-slate-900 border-t-4 border-slate-200 dark:border-slate-700 flex lg:hidden items-center justify-around min-h-[3.5rem] sm:min-h-16 pb-safe safe-area-bottom shadow-[0_-4px_10px_0_rgba(0,0,0,0.1)] dark:shadow-[0_-4px_10px_0_rgba(0,0,0,0.3)] transform transition-transform duration-300 ease-out ${bottomNavVisible ? 'translate-y-0' : 'translate-y-32'}`}>
          {canManage && (
            <button
      ...
      {mobileMenuOpen && (
        <>
          ...
          <div className="fixed inset-y-0 left-0 z-[70] w-3/4 max-w-[280px] bg-white dark:bg-slate-900 border-r-4 border-slate-200 dark:border-slate-700 flex flex-col shadow-2xl lg:hidden animate-in slide-in-from-left duration-300">
            <div className="p-5 border-b-2 border-slate-100 dark:border-slate-700 flex items-center justify-between">
              ...
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-lg border-2 border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white active:bg-slate-50 dark:active:bg-slate-700 transition-all"
              >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {classes.map((c, idx) => {
                ...
                if (isEditing) {
                  return (
                    <div key={c.id} className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border-2 border-slate-200 dark:border-slate-700 shadow-[4px_4px_0_0_#334155] dark:shadow-[4px_4px_0_0_#1e293b] flex flex-col gap-3 animate-in zoom-in-95 duration-200">
                      <div className="flex flex-col gap-1.5 w-full">
                        ...
                        <input
                          ...
                          className="w-full px-4 py-2.5 rounded-xl bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-600 text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 bg-indigo-500 text-white text-[10px] font-black uppercase tracking-widest py-3 rounded-xl border-2 border-slate-200 shadow-[4px_4px_0_0_#334155] active:shadow-none active:translate-x-0.5 active:translate-y-0.5"
                        >
                          Simpan
                        </button>
                        <button
                          onClick={() => setMobileEditingClassId(null)}
                          className="flex-1 bg-white text-slate-500 text-[10px] font-black uppercase tracking-widest py-3 rounded-xl border-2 border-slate-200"
                        >
                          Batal
                        </button>
                      </div>
                ...
                return (
                  <div
                    key={c.id}
                    className={`relative flex items-center gap-2 p-1.5 rounded-2xl border-2 transition-all ${isActive ? 'bg-amber-300 dark:bg-amber-700 border-slate-200 dark:border-slate-600 shadow-[4px_4px_0_0_#334155] dark:shadow-[4px_4px_0_0_#1e293b] -translate-y-0.5' : 'hover:bg-slate-50 dark:hover:bg-slate-800 border-slate-100 dark:border-slate-700'}`}
                  >
                    <button
                      onClick={() => {
                    ...
                    {canManage && (
                      <div className="flex items-center gap-1.5 pr-1.5">
                        <button
                          ...
                          className={`w-8 h-8 flex items-center justify-center rounded-lg border-2 border-slate-200 dark:border-slate-600 active:scale-90 transition-all ${isActive ? 'bg-white dark:bg-slate-800' : 'bg-slate-100 dark:bg-slate-800'}`}
                        >
                        ...
                        <button
                          ...
                          className={`w-8 h-8 flex items-center justify-center rounded-lg border-2 border-slate-200 dark:border-slate-600 active:scale-90 transition-all ${isActive ? 'bg-white dark:bg-slate-800 text-red-500' : 'bg-red-50 dark:bg-red-950/50 text-red-500'}`}
                        >
            ...
            {/* Bottom Section */}
            <div className="mt-auto p-4 border-t-4 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 space-y-4">
              {/* Profile Status */}
              ...
              {isOwner && currentClass && !myAccessByClass[currentClass.id] && (
                <div className="p-4 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl shadow-[4px_4px_0_0_#334155] dark:shadow-[4px_4px_0_0_#1e293b] space-y-3">
                  {(() => {
                    ...
                    return (
                        ...
                        <div className="flex items-start gap-2.5">
                          <div className="p-2 bg-indigo-100 dark:bg-indigo-900/50 rounded-xl border-2 border-slate-200 dark:border-slate-600 text-indigo-600 dark:text-indigo-400">
                        ...
                        <button
                          ...
                          className="w-full py-2.5 sm:py-3 rounded-lg sm:rounded-xl bg-indigo-500 text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest hover:shadow-[4px_4px_0_0_#334155] dark:hover:shadow-[4px_4px_0_0_#334155] active:translate-y-0.5 transition-all border-2 border-slate-200 dark:border-slate-600"
                        >
                          Daftar di Kelas
                        </button>
                      </div>
                ...
                <div className="w-full">
                  {!addingClass ? (
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 py-2.5 sm:py-3.5 px-3 sm:px-4 rounded-lg sm:rounded-xl bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-600 text-slate-900 dark:text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest active:translate-y-0.5 transition-all hover:bg-slate-50 dark:hover:bg-slate-700"
                    >
                      <Plus className="w-3.5 h-3.5 sm:w-4.5 sm:h-4.5" strokeWidth={3} />
                      Nama kelas
                    </button>
                  ) : (
                    <div className="p-4 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl shadow-[4px_4px_0_0_#334155] dark:shadow-[4px_4px_0_0_#1e293b] animate-in slide-in-from-bottom-2 duration-200">
                      ...
                      <input
                        ...
                        className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-600 text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:bg-white dark:focus:bg-slate-800 transition-all mb-3 dark:placeholder:text-slate-500"
                        autoFocus
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 py-3 rounded-xl bg-emerald-400 dark:bg-emerald-600 text-slate-900 dark:text-white text-[10px] font-black uppercase tracking-widest border-2 border-slate-200 dark:border-slate-600 disabled:opacity-50 shadow-[4px_4px_0_0_#334155] dark:shadow-[4px_4px_0_0_#1e293b] active:shadow-none"
                        >
                          Simpan
                        </button>
                        <button
                          ...
                          className="flex-1 py-3 rounded-xl bg-white dark:bg-slate-800 text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-widest border-2 border-slate-200 dark:border-slate-600"
                        >
                          Batal
                        </button>
                      </div>
