...
import { asObject, asStringArray, asString } from '@/components/yearbook/utils/response-narrowing'
...
export default function TryonForm() {
  ...
  const handleTryOn = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const resultList = asStringArray(data.results)
      if (data.ok === true && resultList.length > 0) {
        // Chain mode returns only 1 result (final combined image with all garments)
        // Show the final result (1 foto dengan semua garment)
        const finalResult = resultList[0]; // Chain mode returns array with 1 element (final result)
        ...
        setError(asString(data.error) || "Error processing garments");
      }
    } catch (err) {
