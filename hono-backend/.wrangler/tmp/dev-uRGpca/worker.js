...
var init_phoenix = __esm({
  "node_modules/@supabase/phoenix/priv/static/phoenix.mjs"() {
    ...
    WS_CLOSE_NORMAL = 1e3;
    SOCKET_STATES = /** @type {const} */
    { connecting: 0, open: 1, closing: 2, closed: 3 };
    CHANNEL_STATES2 = /** @type {const} */
    {
      closed: "closed",
      errored: "errored",
      joined: "joined",
    ...
    TRANSPORTS = /** @type {const} */
    {
      longpoll: "longpoll",
      websocket: "websocket"
    };
    XHR_STATES = /** @type {const} */
    {
      complete: 4
    };
...
function getAdminSupabaseClient(env) {
  const e = env || typeof globalThis !== "undefined" && globalThis.env || (typeof process !== "undefined" ? process.env : {});
...
webhooksXendit.post("/", async (c) => {
  ...
  const specificChannel = raw2?.payment_channel || raw2?.bank_code || raw2?.retail_outlet_name || raw2?.ewallet_type;
  ...
  if (status === "EXPIRED") {
    const { error: txError } = await supabase.from("transactions").update({ status }).eq("external_id", externalId);
...
showcase.get("/", async (c) => {
  try {
    ...
    const { data, error } = await admin.from("site_settings").select("value").eq("key", SHOWCASE_KEY).maybeSingle();
    if (error) {
      return c.json(defaultShowcase);
    }
    const raw2 = data?.value;
    if (!raw2 || typeof raw2 !== "object") return c.json(defaultShowcase);
    const albumPreviews = Array.isArray(raw2.albumPreviews) ? raw2.albumPreviews : defaultShowcase.albumPreviews;
    const flipbookPreviewUrl = typeof raw2.flipbookPreviewUrl === "string" ? raw2.flipbookPreviewUrl : defaultShowcase.flipbookPreviewUrl;
