...
| 3      | `migrations/0003_seed_ref_indonesia.sql` | 34 provinsi + 514 kab/kota (`ref_*`). Dibangkitkan dari `scripts/ref-indonesia-wilayah.postgres.sql` via `npm run d1:seed-wilayah`. |
...
**Konvensi:** `user_id` di D1 = string ID user (Firebase `uid`). Boolean disimpan sebagai `INTEGER` 0/1.
