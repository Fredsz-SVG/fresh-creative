...
-- ----------------------------------------------------------------------------
-- 3. ALBUM CLASS REQUESTS (Legacy Support)
-- ----------------------------------------------------------------------------
-- Maintained for API compatibility if code still separates pending requests
CREATE TABLE IF NOT EXISTS public.album_class_requests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  class_id uuid NOT NULL REFERENCES public.album_classes(id) ON DELETE CASCADE,
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  student_name text NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_album_class_requests_class_id ON public.album_class_requests(class_id);
ALTER TABLE public.album_class_requests ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "Insert requests" ON public.album_class_requests;
CREATE POLICY "Insert requests" ON public.album_class_requests FOR INSERT WITH CHECK (
